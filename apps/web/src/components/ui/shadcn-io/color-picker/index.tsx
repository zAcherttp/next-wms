...
export const ColorPicker = ({
  ...
  const onColorChange = useCallback(
    (
      ...
      if (onChange) {
        ...
        onChange([rgba[0], rgba[1], rgba[2], newAlpha / 100]);
      }
    },
    [onChange]
  );
  ...
  const handleHueChange = useCallback(
    (newHue: number) => {
      setHue(newHue);
      onColorChange(newHue, saturation, lightness, alpha);
    },
    [saturation, lightness, alpha, onColorChange]
  );

  const handleSaturationChange = useCallback(
    (newSaturation: number) => {
      setSaturation(newSaturation);
      onColorChange(hue, newSaturation, lightness, alpha);
    },
    [hue, lightness, alpha, onColorChange]
  );

  const handleLightnessChange = useCallback(
    (newLightness: number) => {
      setLightness(newLightness);
      onColorChange(hue, saturation, newLightness, alpha);
    },
    [hue, saturation, alpha, onColorChange]
  );

  const handleAlphaChange = useCallback(
    (newAlpha: number) => {
      setAlpha(newAlpha);
      onColorChange(hue, saturation, lightness, newAlpha);
    },
  ...
  const setHsla = useCallback(
    (
      ...
      setLightness(newLightness);
      setAlpha(newAlpha);
      onColorChange(newHue, newSaturation, newLightness, newAlpha);
    },
    [onColorChange]
  );

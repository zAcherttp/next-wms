...
export const list = query({
    ...
    handler: async (ctx, args) => {
        const { organizationId, paginationOpts, isActive, includeDeleted = false } = args;
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Generate path
        const path = parentPath ? `${parentPath}.${name.toLowerCase().replace(/\s+/g, "_")}` : name.toLowerCase().replace(/\s+/g, "_");
...
export const remove = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (hasProducts) {
            throw new Error("Cannot delete category that has products. Remove products first.");
        }

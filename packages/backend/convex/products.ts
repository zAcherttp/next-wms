...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        // Apply filters
        if (categoryId) {
            queryBuilder = queryBuilder.filter((q) => q.eq(q.field("categoryId"), categoryId));
        }

        if (brandId) {
            queryBuilder = queryBuilder.filter((q) => q.eq(q.field("brandId"), brandId));
        }
...
export const search = query({
    ...
    handler: async (ctx, args) => {
        ...
        const productIdsFromVariants = new Set(matchedVariants.map((v) => v.productId));
...
export const createVariant = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const { productId, skuCode, temperatureSensitive = false, isActive = true, ...rest } = args;
